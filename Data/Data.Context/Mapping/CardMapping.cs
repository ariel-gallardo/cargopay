...
namespace Data
{
    internal static class CardMapping
    {
        internal static void Map<T>(this ModelBuilder modelBuilder) where T : Card
        {
            ...
            modelBuilder.Entity<T>().Property(x => x.Id).ValueGeneratedOnAdd().HasDefaultValueSql(@"RIGHT(REPLICATE('0', 15) + CAST(CAST(RAND(CHECKSUM(NEWID())) * 1000000000000000 AS BIGINT) AS VARCHAR(15)), 15)");
